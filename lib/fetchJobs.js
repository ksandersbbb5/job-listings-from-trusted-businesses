...
function parseCSV(text) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (inQuotes) {
      ...
      else if (c === '\r') { /* ignore CR; LF ends the row */ }
      else { field += c; }
    }
  }
...
async function fetchRaw() {
  ...
  // If HTML sneaks in, fail loudly so you know the URL is wrong
  if (ct.includes('text/html') || /^<!doctype html/i.test(text) || /<html/i.test(text)) {
    throw new Error('SHEET_URL returned HTML. Publish CSV/JSON and use that direct link.');
  }
  ...
  // Google Sheets published CSV → parse as CSV
...
export async function getJobs() {
  ...
  const jobs = rows.map((row, i) => {
    ...
    // Common header names from Sheets (normalized):
    // business_name, job_title, title, position, city, state, zip, postal_code, description, apply_url, url, website, etc.
    ...
    const baseForId = [businessName, title, n.city, n.state].filter(Boolean).join('-');
  ...
  // Keep rows that have at least a title or a business
  return jobs.filter((j) => j.title || j.businessName);
}

// Optional: used by /api/debug-rows if you add it
