  ...
  return (
    <main className="grid" style={{ gap: '1.25rem' }}>
      <section className="card">
        <form
          className="grid"
          style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem' }}
      ...
      <section className="grid" style={{ gap: '0.75rem' }}>
        ...
        {!loading && !error && filtered.length === 0 && <div className="card">No jobs match your search yet. Try clearing filters.</div>}
        <div className="jobs-grid">
          {filtered.map(job => (
            <article key={job.id} className="card">
              <div className="pill" title="BBB Member">
                BBB Accredited
              </div>
              ...
              {job.description && (
                <p style={{ marginTop: 6, color: '#20404a' }}>
                  {job.description.slice(0, 160)}
                  {job.description.length > 160 ? '…' : ''}
                </p>
              )}
              <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                <a className="button" href={`/job/${job.id}`}>
                  View Details
                </a>
                {job.applyUrl && (
                  <a className="button secondary" href={job.applyUrl} target="_blank" rel="noopener noreferrer">
                    Apply
                  </a>
                )}
              </div>
