...
export default function JobDetailPage({ job }: InferGetServerSidePropsType<typeof getServerSideProps>) {
  ...
  const jsonLd = {
    ...
    hiringOrganization: { '@type': 'Organization', name: job.businessName, sameAs: job.businessUrl || undefined },
    jobLocation:
      job.city || job.state || job.postalCode
        ? { '@type': 'Place', address: { '@type': 'PostalAddress', streetAddress: job.streetAddress, addressLocality: job.city, addressRegion: job.state, postalCode: job.postalCode, addressCountry: 'US' } }
        : undefined,
    baseSalary: job.baseSalary ? { '@type': 'MonetaryAmount', currency: 'USD', value: { '@type': 'QuantitativeValue', value: job.baseSalary } } : undefined,
    applicantLocationRequirements: job.city || job.state ? { '@type': 'Country', name: 'United States' } : undefined,
      ...
      <main className="grid" style={{ gap: '1rem' }}>
        <article className="card">
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', margin: '0.35rem 0' }}>
            {(job.city || job.state || job.postalCode) && <span className="badge">{[job.city, job.state, job.postalCode].filter(Boolean).join(', ')}</span>}
